...
export default function Projects() {
  ...
  const projectsList = Object.keys(t.projects)
    ...
    <section id="projets" className="scroll-mt-36">
       ...
       <div className="grid grid-cols-1 gap-12">
          ...
          {projectsList.map((project) => {
            // NOUVELLE LOGIQUE : On détermine si le projet est "actif" (en développement)
            // Seul p1 clignotera. p2 sera fixe.
            const isProjectActive = project.id === 'p1';

            return (
            <div key={project.id || project.title} className="group relative rounded-2xl bg-[var(--card-bg)] border border-[var(--card-border)] hover:border-[#00CCFF]/50 transition-all duration-300 overflow-hidden">
               ...
               <div className="relative p-6 md:p-8">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6">
                     ...
                     <div className="flex items-center gap-4">
                         <img 
                             src={projectImages[project.id] || "/images/DZ25.png"}
                     ...
                     <div className="flex items-center gap-2 mt-1 md:mt-0">
                        <span className="relative flex h-2.5 w-2.5">
                          {/* MODIFICATION ICI : L'animation 'ping' n'est affichée QUE SI isProjectActive est vrai */}
                          {isProjectActive && (
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#00CCFF] opacity-75"></span>
                          )}
                          {/* Le point fixe, lui, est toujours là */}
