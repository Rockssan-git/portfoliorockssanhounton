  ...
  return (
    <section id="experiences" className="scroll-mt-36">
      <h3 className="text-xl font-bold mb-6 flex items-center gap-2" style={{color: 'var(--text-primary)'}}>
        <span className="text-[#00CCFF]">⚡</span> {t.experience.title}
      </h3>

      <div className="space-y-6">
        {experiences.map((exp) => (
          <div key={exp.id} className={`relative p-6 rounded-2xl border transition-all duration-300 group ${exp.highlight ? 'bg-[var(--card-bg)] border-[#00CCFF] shadow-[0_0_15px_rgba(0,204,255,0.1)]' : 'bg-[var(--card-bg)] border-[var(--card-border)] hover:border-[#00CCFF]/50'}`}>
            <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-2 mb-2">
                ...
                {exp.period && (
                    <span className={`text-xs font-bold px-2 py-1 rounded uppercase tracking-wider ${exp.highlight ? 'bg-[#00CCFF] text-black' : 'bg-[#00CCFF]/10 text-[#00CCFF] border border-[#00CCFF]/30'}`}>
                        {exp.period}
                    </span>
                )}
            </div>
